...
function SidebarProvider({
  ...
  const [sidebarWidth, setSidebarWidthState] = React.useState(() => {
    ...
    const parsedWidth = Number(widthCookie)
  ...
  const setSidebarWidth = React.useCallback((width: number) => {
    const nextWidth = Math.min(SIDEBAR_WIDTH_MAX, Math.max(SIDEBAR_WIDTH_MIN, Math.round(width)))
...
function Sidebar({
  ...
  }, [setSidebarWidth, side, sidebarWidth, state])
  ...
  return (
    <div
      className="group peer text-sidebar-foreground hidden overflow-x-hidden md:block"
      data-state={state}
    ...
    >
      ...
      <div
        className={cn(
          "relative h-svh w-(--sidebar-width) overflow-hidden bg-transparent transition-[width] duration-200 ease-linear",
          "w-[var(--sidebar-width-current)]",
      ...
      <div
        className={cn(
          ...
          "w-[var(--sidebar-width-current)]",
          side === "left"
            ? "left-0 group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width-current)*-1)]"
      ...
      >
        ...
        <button
          ...
          className={cn(
            "absolute inset-y-0 z-30 hidden w-3 -translate-x-1/2 cursor-col-resize md:flex",
            side === "left" ? "-right-1.5" : "-left-1.5",
            state === "collapsed" ? "pointer-events-none opacity-0" : "opacity-100"
          )}
        >
          <span className="sidebar-resize-handle__line" />
        </button>
      </div>
    </div>
  )
