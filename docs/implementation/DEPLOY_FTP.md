...
GitHub Actions по FTP может автоматизировать заливку файлов. Миграции можно запускать из GitHub Actions отдельным ручным флагом только если MySQL хостинга доступен извне для GitHub runner. Это не заменяет SSH полностью: `storage:link`, права папок, server cache и rollback БД без SSH остаются ручными.
...
2. В GitHub Actions variables добавить `PRODUCTION_APP_URL`.
