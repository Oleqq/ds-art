...
GitHub Actions по FTP может автоматизировать заливку файлов, но без SSH не решает миграции, `storage:link`, cache и rollback БД.
...
Для первого тестового выката через FTP база будет в порядке только в одном из двух случаев:
...
Если доступа к панели хостинга и к БД сейчас нет, то корректный первый выклад только по FTP без отдельной подготовки БД гарантировать нельзя.
...
Если хостинг позволяет выбрать document root, еще лучше поставить корень сайта сразу на:

```text
ds-art-app/public
```

Тогда `index.php` менять не нужно.
